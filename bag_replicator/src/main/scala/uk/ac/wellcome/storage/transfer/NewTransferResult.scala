...
case class NewPrefixTransferFailure(failures: Int,
                                 successes: Int,
                                 e: Throwable = new Error())
  extends NewTransferFailure

case class NewPrefixTransferListingFailure[Prefix](prefix: Prefix,
                                                e: Throwable = new Error())
  extends NewTransferFailure

sealed trait NewTransferSuccess extends NewTransferResult

case class NewTransferNoOp[SrcLocation, DstLocation](source: SrcLocation,
                                                  destination: DstLocation)
  extends NewTransferSuccess

case class NewTransferPerformed[SrcLocation, DstLocation](source: SrcLocation,
                                                       destination: DstLocation)
  extends NewTransferSuccess
