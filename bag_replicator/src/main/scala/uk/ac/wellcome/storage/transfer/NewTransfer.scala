...
trait NewTransfer[SrcLocation <: Location, DstLocation <: Location] {
  ...
  def transfer(
    ...
    underlying.transfer(src.toObjectLocation, dst.toObjectLocation) match {
      ...
      case Right(TransferPerformed(_, _)) =>
        Right(NewTransferPerformed(src, dst))
      case Left(TransferSourceFailure(_, _, e)) =>
        Left(NewTransferSourceFailure(src, dst, e))
      case Left(TransferDestinationFailure(_, _, e)) =>
        Left(NewTransferDestinationFailure(src, dst, e))
      case Left(TransferOverwriteFailure(_, _, e)) =>
        Left(NewTransferOverwriteFailure(src, dst, e))
      case _ =>
        throw new RuntimeException(
          "This is a PrefixTransferResult, which shouldn't be returned here"
        )
    }
}
