...
case class UnpackerStorageError(storageError: StorageError) extends UnpackerError {
  override val e: Throwable = storageError.e
}

case class UnpackerUnarchiverError(unarchiverError: UnarchiverError) extends UnpackerError {
