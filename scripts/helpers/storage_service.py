...
def lookup_ingest_id(environment, space, external_identifier, version):
    ...
    matching_manifests = list(list_s3_prefix(
        s3, bucket=manifests_bucket, prefix=f"{space}/{external_identifier}/{version.replace('v', '')}/"
    ))

    if len(matching_manifests) >= 1:
        manifest_key = matching_manifests[0]

        manifest = json.load(s3.get_object(Bucket=manifests_bucket, Key=manifest_key)["Body"])

        return manifest["ingestId"]
    else:
        sys.exit(f"Unable to find ingest ID from manifests bucket! ({len(matching_manifests)} manifests found)")
