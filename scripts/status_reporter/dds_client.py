...
class DDSClient:
    def __init__(self, start_ingest_url, item_query_url, connection_pool_size=1):
        ...
        self.http_adapter = requests.adapters.HTTPAdapter(
            pool_connections=1, pool_maxsize=25, pool_block=True
        )
    ...
    def _request_json(self, url):
        ...
        return {"body": body, "status": status, "notes": notes}
    ...
    def status(self, bnumber):
        ...
        if status_result["status"] is "ok":
            ...
            if waiting and finished:
                raise Exception(
                    f"Impossible status for {bnumber}, waiting AND finished!"
                )
            elif waiting:
                result = "waiting"
            elif finished:
        ...
        return {"bnumber": bnumber, "status": result}
