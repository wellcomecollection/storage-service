...
class ManifestSync:
    ...
    def _match_images(self, image_summary, bnumber):
        ...
        def _files_from_storage_service(storage_client, bnumber):
            ...
            return {
                _last_segment(f["path"]): f["path"]
                for f in files
                if f["name"].startswith("data/objects/")
            }
        ...
        matched = {
            path: _find_key(i, keyed)
            for i, path in ss_files.items()
            if _find_key(i, keyed)
        }
    ...
    def _get_images_summary(self, differences):
        ...
        for image in differences["images"]:
            if "https://dlcs.io/iiif-img" in image["new_value"] and not image[
                "new_value"
                ...
                image_summary = {"old": old_image_id, "new": new_image_id}
    ...
    def _compare_asset_length(self, images_summary, bnumber):
        ...
        for new_id, old_id in images_summary.items():
            ...
            new_content_length = _try(
                _get_storage_service_object_content_length, new_id
            )
            ...
            if not size_match:
                differences.append(
                    {
                        "old": {"id": old_id, "size": old_cl},
                        "new": {"id": new_id, "size": new_cl},
                    }
                )
    ...
    def diff_summary_all_finished(self, status="finished"):
        ...
        for batch_summary in self._batch_summary(status, batch_size):
            ...
            mismatched = {
                bnumber: item
                for bnumber, item in batch_summary.items()
                if _has_mismatch(item)
            }
    ...
    def retry_mismatched_manifests(self, status="finished"):
        ...
        for batch_summary in self._batch_summary(status, batch_size):
            ...
            print(
                f"Found {mismatch_count} mismatched manifests, total: {total_mismatch}"
            )
