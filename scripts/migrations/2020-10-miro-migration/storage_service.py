...
def get_latest_ingest(space, external_identifier):
    ...
    ingests_index_query = {
        "query": {
            "bool": {
                ...
                "must": {"prefix": {"bag.info.externalIdentifier": {"value": external_identifier}}},
            }
        }
    }

    response = elastic_client.search(
        index=REPORTING_INGESTS_INDEX,
        body=ingests_index_query
    )
    ...
    for hit in response['hits']['hits']:
        ...
        parsed_date = datetime.datetime.strptime(lastModifiedDate, "%Y-%m-%dT%H:%M:%S.%fZ")
...
def get_bag(space, external_identifier):
    ...
    bags_index_query = {
        "query": {
            "bool": {
                ...
                "must": {
                    "term": {"bag.info.externalIdentifier": {"value": external_identifier}}
                },
            }
        },
        "size": 50
    ...
    response = elastic_client.search(
        index=REPORTING_BAGS_INDEX,
        body=bags_index_query
    )
