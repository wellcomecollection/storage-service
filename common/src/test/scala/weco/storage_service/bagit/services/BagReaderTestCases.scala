...
trait BagReaderTestCases[
  ...
  it("gets a correctly formed bag with multiple manifests") {
    withFixtures { fixtures =>
      implicit val (context, typedStore, namespace, bucket) = fixtures
  ...
  it("errors if there aren't any payload manifests") {
    withFixtures { fixtures =>
      implicit val (context, typedStore, namespace, bucket) = fixtures
  ...
  it("errors if there aren't any tag manifests") {
    withFixtures { fixtures =>
      implicit val (context, typedStore, namespace, bucket) = fixtures
  ...
  it("errors if different tag manifests list different files") {
    withFixtures { fixtures =>
      implicit val (context, typedStore, namespace, bucket) = fixtures
  ...
  it("errors if the bag only uses weak algorithms") {
    withFixtures { fixtures =>
      ...
      err.msg shouldBe "Payload manifests only use deprecated checksums: add a payload manifest using SHA-256 or SHA-512"
    }
  }
