...
class ChecksumValueTest extends AnyFunSpec with Matchers with TableDrivenPropertyChecks {
  ...
  it("treats two equivalent checksums as equal") {
    forAll(equalityTestCases) { case (value1, value2) =>
  ...
  it("treats two different checksums as not-equal") {
    forAll(differingTestCases) { case (value1, value2) =>
      val checksum1 = ChecksumValue(value1)
      val checksum2 = ChecksumValue(value2)
