...
import uk.ac.wellcome.platform.archive.common.operation.{IngestUpdater, DiagnosticReporter, OutgoingPublisher}

trait OperationFixtures extends SNS with MetricsSenderFixtures {
  def withIngestUpdater[R](operationName: String, topic: Topic)(testWith: TestWith[IngestUpdater, R]): R =
  ...
  def withOutgoingPublisher[R](operationName: String, topic: Topic)(testWith: TestWith[OutgoingPublisher, R]): R =
    withSNSWriter(topic) { snsWriter =>
