...
class AzureLargeStreamReaderTest extends LargeStreamReaderTestCases[AzureBlobLocation, Container] with AzureFixtures {
  ...
  override def withLargeStreamReader[R](
    ...
    testWith: TestWith[LargeStreamReader[AzureBlobLocation],
      R]): R =
    testWith(
      new AzureLargeStreamReader(bufferSize = bufferSize) {
        override val rangedReader: RangedReader[AzureBlobLocation] = rangedReaderImpl
      }
    )
}
