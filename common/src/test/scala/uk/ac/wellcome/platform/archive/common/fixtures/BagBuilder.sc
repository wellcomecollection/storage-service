...
import uk.ac.wellcome.platform.archive.common.generators.{BagInfoGenerators, ExternalIdentifierGenerators, StorageSpaceGenerators}
...
object BagBuilder extends StorageSpaceGenerators with BagInfoGenerators {
  ...
  private def createPayloadFiles(
    ...
    externalIdentifier: ExternalIdentifier = createExternalIdentifier,
    version: BagVersion = BagVersion(randomInt(from = 2, to = 10)),
    payloadFileCount: Int): Seq[PayloadEntry] = {
    val bagRoot = createBagRoot(space, externalIdentifier, version)
