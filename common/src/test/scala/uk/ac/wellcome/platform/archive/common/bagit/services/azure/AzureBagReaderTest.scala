...
class AzureBagReaderTest extends BagReaderTestCases[
  ...
  override def asLocation(root: AzureBlobLocationPrefix, path: String): AzureBlobLocation = root.asLocation(path)
  ...
  override def deleteFile(root: AzureBlobLocationPrefix, path: String)(implicit context: Unit): Unit = {
    azureClient.getBlobContainerClient(root.container).getBlobClient(s"${root.namePrefix}/$path").delete()
  }
}
