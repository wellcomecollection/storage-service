...
class AzureBlobSizeFinderTest extends SizeFinderTestCases[AzureBlobLocation, Container]
  with AzureFixtures {
  override def withContext[R](testWith: TestWith[Container, R]): R = withAzureContainer{ container => testWith(container)}

  override def withSizeFinder[R](testWith: TestWith[SizeFinder[AzureBlobLocation], R])(implicit context: Container): R = testWith(new AzureSizeFinder())
