...
class IngestsWorkerServiceTest
  ...
  describe("When the client succeeds") {
    it("returns Successful") {
      withIngestWorker(ingestTrackerClient = successfulClient(ingest)) { worker =>
        whenReady(worker.processMessage(ingestStatusUpdate)) { result: Result[Ingest] =>
  ...
  describe("When the client return a conflict") {
    it("returns DeterministicFailure") {
      withIngestWorker(ingestTrackerClient = conflictClient(ingestStatusUpdate)) { worker =>
        whenReady(worker.processMessage(ingestStatusUpdate)) { result: Result[Ingest] =>
  ...
  describe("When the client return an unknown error") {
    it("returns NonDeterministicFailure") {
      withIngestWorker(ingestTrackerClient = unknownErrorClient(ingestStatusUpdate)) { worker =>
        whenReady(worker.processMessage(ingestStatusUpdate)) { result: Result[Ingest] =>
