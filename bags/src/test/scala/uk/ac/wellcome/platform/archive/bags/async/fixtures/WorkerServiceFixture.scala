...
trait WorkerServiceFixture extends Akka with RandomThings with SNS with SQS with UpdateStoredManifestServiceFixture {
  ...
  def createReplicationResultWith(archiveBagLocation: BagLocation): ReplicationResult =
    ReplicationResult(
