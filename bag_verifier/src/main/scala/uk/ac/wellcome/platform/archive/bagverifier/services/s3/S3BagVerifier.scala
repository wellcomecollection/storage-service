...
object S3BagVerifier{
  def standalone( primaryBucket: String)(implicit  s3Client: AmazonS3): S3StandaloneBagVerifier = {
  ...
  def replicated( primaryBucket: String)(implicit  s3Client: AmazonS3): S3ReplicatedBagVerifier = {
    val bagReader = new S3BagReader()
    val listing = S3ObjectLocationListing()
    val resolvable = new S3Resolvable()
    implicit val fixityChecker=new S3FixityChecker()
