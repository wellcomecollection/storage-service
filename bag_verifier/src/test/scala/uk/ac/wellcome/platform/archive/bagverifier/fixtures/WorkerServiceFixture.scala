...
import uk.ac.wellcome.platform.archive.bagverifier.services.{NewBagVerifierWorker, Verifier}
...
trait WorkerServiceFixture
  ...
  def withBagVerifierWorker[R](ingestTopic: Topic,
    ...
    testWith: TestWith[NewBagVerifierWorker, R]): R =
        ...
        withMaterializer(actorSystem) { implicit materializer =>
          ...
          withIngestUpdater("verification", ingestTopic) { ingestUpdater =>
            withOutgoingPublisher("verification", outgoingTopic) {
              outgoingPublisher =>
                val service = new NewBagVerifierWorker(
