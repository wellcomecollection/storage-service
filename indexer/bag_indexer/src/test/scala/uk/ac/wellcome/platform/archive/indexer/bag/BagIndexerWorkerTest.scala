...
class BagIndexerWorkerTest
  ...
  def withStoreReadErrorIndexerWorker[R](index: Index, queue: SQS.Queue)(
    ...
    withActorSystem { implicit actorSystem =>
      withFakeMonitoringClient() { implicit monitoringClient =>
        val storageManifestDao: StorageManifestDao = new MemoryStorageManifestDao(
  ...
  it("fails with a NonDeterministicFailure when a StoreReadError is encountered") {
    val (t, _) = createT
  ...
  it("fails with a DeterministicFailure when a DoesNotExistError is encountered") {
    val (t, _) = createT
    withLocalElasticsearchIndex(mapping) { index =>
      withLocalSqsQueue() { queue =>
