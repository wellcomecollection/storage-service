...
class BagIndexerTest
  ...
  override def createIndexer(client: ElasticClient, index: Index): Indexer[StorageManifest, IndexedStorageManifest] =
  ...
  override def assertMatch(indexedDocument: IndexedStorageManifest, expectedDocument: StorageManifest): Assertion =
