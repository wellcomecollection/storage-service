...
import uk.ac.wellcome.platform.archive.common.storage.models.{StorageLocation, StorageManifestFile}
import uk.ac.wellcome.storage.{ObjectLocation, ObjectLocationPrefix, UpdateError}
...
class ApplyTags(s3Tags: S3Tags) extends Logging {
  def applyTags(storageLocations: Seq[StorageLocation], tagsToApply: Map[StorageManifestFile, Map[String, String]]): Try[Unit] =
    Try {
        ...
        storageLocations.flatMap { location =>
          location.provider match {
            case AmazonS3StorageProvider =>
              applyTags(s3Tags, prefix = location.prefix, tagsToApply = tagsToApply)
            case provider =>
              throw new IllegalArgumentException(s"Unsupported provider for tagging: $provider")
          }
        }
  ...
  private def applyTags(tags: Tags[ObjectLocation], prefix: ObjectLocationPrefix, tagsToApply: Map[StorageManifestFile, Map[String, String]])
    : Iterable[Either[UpdateError, Unit]] =
    tagsToApply
      .map { case (storageManifestFile, newTags) =>
        debug(s"Applying tags: provider=$tags, prefix=$prefix, path=${storageManifestFile.path}, tags=$tagsToApply")
