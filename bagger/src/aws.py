...
def upload(source, key):
    s3_client.upload_file(source, settings.DROP_BUCKET_NAME, key)

    return {"bucket": settings.DROP_BUCKET_NAME, "key": key}


def get_mets_xml(s3_path):
    obj = s3_resource.Object(settings.METS_BUCKET_NAME, s3_path)
...
def save_mets_to_side(b_number, local_tmp_file):
    ...
    s3_client.upload_file(local_tmp_file, settings.DROP_BUCKET_NAME_METS_ONLY, key)
...
def save_id_map(b_number, id_map):
    ...
    obj = s3_resource.Object(settings.DROP_BUCKET_NAME_METS_ONLY, s3_path)
...
def log_processing_error(message):
    ...
    obj = s3_resource.Object(settings.DROP_BUCKET_NAME_ERRORS, s3_path)
...
def send_bag_instruction(message):
    queue = None
    try:
        queue = sqs_resource.get_queue_by_name(QueueName=settings.BAGGING_QUEUE)
...
def get_error_for_b_number(bnumber):
    try:
        obj = s3_resource.Object(settings.DROP_BUCKET_NAME_ERRORS, bnumber + ".json")
...
def get_all_errors():
    bucket = s3_resource.Bucket(settings.DROP_BUCKET_NAME_ERRORS)
...
def get_dropped_bag_info(bnumber):
    ...
    try:
        bag_head = s3_client.head_object(Bucket=settings.DROP_BUCKET_NAME, Key=key)
    except ClientError as e:
