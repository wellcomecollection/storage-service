...
class StorageManifestServiceTest
  ...
  describe("checks the replica root paths") {
    describe("primary location") {
      it("fails if the root is not a versioned directory") {
        val location = createNewPrimaryLocationWith(
        ...
        assertIsError(location = location, version = version) { err =>
          ...
          err.getMessage shouldBe s"Malformed bag root: ${location.prefix.toObjectLocationPrefix} (expected suffix /$version)"
        }
      }
  ...
  it("sets the locations and replicaLocations correctly") {
    ...
    withLocalS3Bucket { implicit bucket =>
      ...
      val location = PrimaryS3StorageLocation(prefix = bagRoot)

      val replicas = collectionOf(max = 10) {
        createSecondaryLocationWith(version).asInstanceOf[SecondaryS3StorageLocation]
      }
  ...
  it("if there's no fetch.txt, it versions the paths") {
    ...
    withLocalS3Bucket { implicit bucket =>
      ...
      val location = createNewPrimaryLocationWith(prefix = bagRoot)
  ...
  it("uses the checksums") {
    ...
    withLocalS3Bucket { implicit bucket =>
      ...
      val bagTagManifestChecksums =
        bag.tagManifest.entries
          .map { case (bagPath, checksum) => bagPath.value -> checksum.value }

      tagManifestChecksums.filterKeys { _ != "tagmanifest-sha256.txt" } shouldBe bagTagManifestChecksums
    }
  }
  ...
  describe("adds the size to the manifest files") {
    it("fails if it cannot get the size of a file") {
      ...
      val service = new S3StorageManifestService() {
        override val sizeFinder: SizeFinder[S3ObjectLocation] =
          new SizeFinder[S3ObjectLocation] {
    ...
    it("uses the size finder to get sizes") {
      ...
      withLocalS3Bucket { implicit bucket =>
        ...
        implicit val cachingSizeFinder = new SizeFinder[S3ObjectLocation] {
          override def get(location: S3ObjectLocation): ReadEither = {
  ...
  implicit val streamStore: NewS3StreamStore =
    ...
    it("finds BagIt tag manifest files") {
      ...
      withLocalS3Bucket { implicit bucket =>
        ...
        val manifest = createManifest(
          bag = bag,
          location = createNewPrimaryLocationWith(prefix = bagRoot),
          space = space,
          version = version
        )
  ...
  private def createManifest(
    ...
    replicas: Seq[SecondaryNewStorageLocation] = Seq.empty,
    space: StorageSpace = createStorageSpace,
    version: BagVersion,
    sizeFinderImpl: SizeFinder[S3ObjectLocation] =
      new SizeFinder[S3ObjectLocation] {
        override def get(location: S3ObjectLocation): ReadEither =
    ...
    val service = new S3StorageManifestService() {
      override val sizeFinder: SizeFinder[S3ObjectLocation] = sizeFinderImpl
    }
    ...
    val result = service.createManifest(
      ...
      replicas = replicas.map { loc =>
        SecondaryStorageLocation(
          provider = AmazonS3StorageProvider,
          prefix = loc.prefix.toObjectLocationPrefix
        )
      },
      space = space,
      version = version
  ...
  def createPrimaryLocationWith(version: BagVersion): PrimaryNewStorageLocation =
  ...
  def createPrimaryLocationWith(
    ...
    createNewPrimaryLocationWith(
      prefix = bagRoot.join(version.toString).asInstanceOf[S3ObjectLocationPrefix]
    )
  ...
  def createSecondaryLocationWith(
    ...
    createNewSecondaryLocationWith(
      prefix = createS3ObjectLocationPrefix.join(version.toString).asInstanceOf[S3ObjectLocationPrefix]
    )

  private def assertIsError(
    ingestId: IngestID = createIngestID,
    bag: Bag = createBag,
    location: PrimaryNewStorageLocation = createPrimaryLocationWith(
