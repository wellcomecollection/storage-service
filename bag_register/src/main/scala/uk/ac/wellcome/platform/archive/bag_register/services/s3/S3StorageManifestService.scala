...
import uk.ac.wellcome.storage.{ObjectLocationPrefix, Prefix, S3ObjectLocation, S3ObjectLocationPrefix}
...
class S3StorageManifestService(implicit s3Client: AmazonS3)
  ...
  override def toLocationPrefix(prefix: ObjectLocationPrefix): Prefix[S3ObjectLocation] =
    S3ObjectLocationPrefix(prefix)

  override val sizeFinder: SizeFinder[S3ObjectLocation] =
    new S3SizeFinder()

  override implicit val streamReader: Readable[S3ObjectLocation, InputStreamWithLength] =
