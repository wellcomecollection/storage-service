...
import uk.ac.wellcome.platform.archive.common.operation.{OperationCompleted, OperationFailure, OperationResult}
...
class Register(
  ...
  def update(
    ...
    for {
      ...
      registrationWithBagId =
      registration.copy(bagId = Some(manifest.id))

      completedRegistration <- storageManifestVHS
        .updateRecord(manifest)(_ => manifest).transform {
