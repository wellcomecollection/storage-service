...
class NewBagRegisterWorker(
  ...
  encoder: Encoder[BagRequest]) extends Runnable with Logging with IngestStepWorker {
  ...
  def processMessage(bagRequest: BagRequest): Future[Result[RegistrationSummary]] =
    for {
      registrationSummary <- register.update(bagRequest.bagLocation)
      _ <- ingestUpdater.send(bagRequest.requestId, registrationSummary, bagId = registrationSummary.summary.bagId)
