...
class ReplicaAggregator(versionedStore: VersionedStore[ReplicaIdentifier, Int, Set[ReplicaResult]]) {
  ...
  def aggregate(
    ...
    val aggregation: Either[UpdateError, IngestStepSucceeded[ReplicationAggregationComplete]] = for {
      upsert <- versionedStore.upsert(replicaIdentifier)(Set(replicaResult))(_ + replicaResult)
