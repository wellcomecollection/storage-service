...
import uk.ac.wellcome.platform.archive.common.models.bagit.{BagInfo, ExternalIdentifier}
...
class UnpackedBagService(s3Client: AmazonS3)(implicit ec: ExecutionContext) {
  ...
  def getBagIdentifier(objectLocation: ObjectLocation): Future[ExternalIdentifier] =
    for {
      bagInfoPath: String <- Future.fromTry(s3BagFile.locateBagInfo(objectLocation))
      inputStream: InputStream <- objectLocation.copy(key = bagInfoPath).toInputStream
